...
import {
  ...
  SpeakerHigh, SpeakerSlash, Sparkle, X, Info,
...
function MissionStepView({ step, mission, save, selected, setSelected, resetSelected, investigated, setInvestigated, selectedEvidence, setSelectedEvidence, collectEvidence, elapsed, advance, complete, setSave, onAcademy }: { step: MissionStep; mission: Mission; save: SaveData; selected: number | null; setSelected: (index: number) => void; resetSelected: () => void; investigated: string[]; setInvestigated: React.Dispatch<React.SetStateAction<string[]>>; selectedEvidence: string[]; setSelectedEvidence: React.Dispatch<React.SetStateAction<string[]>>; collectEvidence: (id: string) => void; elapsed: number; advance: () => void; complete: () => void; setSave: React.Dispatch<React.SetStateAction<SaveData>>; onAcademy: () => void }) {
  ...
  if (step.type === "briefing") {
    ...
    const requestText = isM05 ? "과거 영상 삭제 지원" : "기본권 침해 판례 정밀 조사";
    const periodText = isM05 ? "2년 전" : "역사적 발생 사건";
    ...
    return (
      <CaseBriefingTemplate>
        ...
        <div className="briefing-consult-box">
          <CharacterPortrait characterId={briefingPartner} expression={briefingPartner === "zero" ? "challenge" : "guide"} size="avatar" label={false} />
          <p>
            {briefingPartner === "ari" && "탐구관님, 사건의 사실 관계와 핵심 쟁점부터 차근차근 확인해 볼까요?"}
            {briefingPartner === "player" && "네, 관련 자료를 찾아보고 당사자의 진술과 법적 근거를 조사하겠습니다."}
            {briefingPartner === "zero" && "무조건 삭제하는 게 정답은 아니야. 공익성과 알 권리도 따져봐야지."}
          </p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "10px" }}>
          <div className="briefing-cast-row">
            <button className={`cast-avatar-button ${briefingPartner === "ari" ? "active" : ""}`} onClick={() => setBriefingPartner("ari")} aria-label="파트너 아리 선택">
              <CharacterPortrait characterId="ari" expression="smile" size="avatar" label={false} />
            </button>
  ...
  // 04. CHARACTER DIALOGUE (Interactive Duo Layout)
  if (step.type === "dialogue" && step.scene) {
    ...
    return (
      <DialogueTemplate>
        ...
        {step.scene.image && (
          <figure className="textbook-scene">
            <img src={`/assets/sources/${step.scene.image}`} alt={step.scene.imageAlt ?? "교과서 사례 자료"} />
            <figcaption>교과서 핵심 사례·삽화</figcaption>
          </figure>
        )}
        ...
        <div className="duo-dialogue-stage">
          ...
          <div className="dialogue-turn turn-left">
            ...
            <div className="dialogue-bubble">
              ...
              {isPlayer ? "먼저 사실 관계를 정리해 보자. 어떤 정보들이 필요할까?" : step.scene.text}
            </div>
          </div>
          ...
          <div className="dialogue-turn turn-right">
            ...
            <div className="dialogue-bubble">
              ...
              {isPlayer ? step.scene.text : "네, 관련 자료를 찾아보고 당사자 이야기도 들어보겠습니다."}
            </div>
          </div>
        </div>

        <div style={{ marginTop: "auto", display: "grid", gap: "8px" }}>
          <button className="secondary-button" style={{ justifyContent: "flex-start", fontSize: "12px" }} onClick={advance}>
            ◉ 어떤 자료를 조사할 수 있을까?
          </button>
          <button className="secondary-button" style={{ justifyContent: "flex-start", fontSize: "12px" }} onClick={advance}>
            ◉ 잊힐 권리가 무엇인지 알려줘.
  ...
  if (step.type === "investigation") {
    ...
    return (
      <InvestigationTemplate>
        <TemplateHeading eyebrow={`조사 포인트 ${investigated.length}/${need}`} title={step.title} description="조사할 항목을 선택하세요 (자료 직접 분석)." icon="evidence" />
  ...
  // 06. SOURCE DETAIL (With Audio Interview for M05)
  if (step.type === "source") {
    const isM05 = mission.id === "m05";
    return (
      <SourceDetailTemplate>
        <TemplateHeading eyebrow="자료 상세보기" title={step.title} description="출처와 성격을 구분하며 쟁점을 분석하세요." />
        ...
        {isM05 ? (
          <div className="audio-interview-card">
            <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="audio-player-bar">
                  <button className="audio-play-btn" onClick={() => void audioManager.playSfx("ui_click")} aria-label="인터뷰 오디오 재생"><Play size={16} weight="fill" /></button>
            ...
            <div style={{ marginTop: "14px", background: "#051622", padding: "12px", borderRadius: "12px", border: "1px solid #ffffff12" }}>
              <span style={{ fontSize: "11px", fontWeight: 800, color: "var(--teal-soft)", display: "block", marginBottom: "4px" }}>인터뷰 내용</span>
            ...
            <div style={{ marginTop: "10px", background: "#092433", padding: "11px", borderRadius: "11px", border: "1px solid #23d2c333" }}>
              ...
              <p style={{ margin: 0, fontSize: "11px", color: "#bdd3db", lineHeight: 1.55 }}>
                당사자는 개인정보 유출로 인한 심리적, 사회적 피해를 호소하고 있음. 비공익적 사생활 정보에 해당.
              </p>
            </div>
          </div>
        ) : (
          <article className="source-paper">
            <span className="source-stamp">헌법 및 판례 분석</span>
            <p>{step.body}</p>
            <small>통합사회2 학습을 위해 재구성한 판례 및 헌법 자료</small>
          </article>
        )}
      ...
      <PuzzleTemplate>
        ...
        {selected !== null && (
          <div className={correct ? "feedback success" : "feedback warning"}>
            {correct ? "정확한 판단입니다! 개념과 사례를 올바르게 연결했습니다." : "오답입니다. 정답을 외우기보다 개념의 본래 의미를 다시 점검하고 재도전하세요."}
          </div>
        )}
  ...
  if (step.type === "evidence") {
    return (
      <EvidenceTemplate>
        <TemplateHeading eyebrow="RIGHTS CARD VAULT" title={step.title} description="카드를 눌러 수호관 증거 보관함에 영구 등록하세요." icon="evidence" />
        <div className="card-rule">
          <strong>권리 카드 획득 기준</strong>
          <span>① 조사 및 퍼즐 판독 통과</span>
          <span>② 카드를 터치하여 보관함 등록</span>
          <span>③ 필수 카드가 모두 모이면 최종 판정 개방</span>
        </div>
        <div className="evidence-list">
          {evidenceIds.map((id) => (
            <EvidenceCard key={id} data={evidenceCatalog[id]} collected={save.evidence.includes(id)} selected={false} onClick={() => collectEvidence(id)} />
          ))}
        </div>
    ...
    return (
      <DecisionTemplate>
        ...
        <div className="evidence-select-row">
          {available.map((id) => (
            <EvidenceCard key={id} data={evidenceCatalog[id]} collected selected={selectedEvidence.includes(id)} onClick={() => setSelectedEvidence((prev) => prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id])} />
          ))}
        </div>
        <button className="primary-button full-button" disabled={selected === null || selectedEvidence.length < needed} onClick={() => { setSave((prev) => ({ ...prev, studentChoices: { ...prev.studentChoices, [mission.id]: selected ?? 0 }, decisionHistory: [...prev.decisionHistory, { missionId: mission.id, choice: selected ?? 0, evidenceIds: selectedEvidence }] })); void audioManager.playSfx("decision_submit"); advance(); }}>
...
function AcademyScreen({ save, setSave, returnPoint, onReturn }: { save: SaveData; setSave: React.Dispatch<React.SetStateAction<SaveData>>; returnPoint: ReturnPoint; onReturn: () => void }) {
  ...
  if (!room) return <AcademyTemplate><GameHUD missionTitle="탐구 아카데미" level={save.level} exp={save.exp} onBack={onReturn} /><div className="screen-scroll"><TemplateHeading eyebrow="ACADEMY · ROOM 1~3" title="개념을 훈련하는 연구실" description="개념 카드 뒤집기부터 주장·근거 글쓰기까지 4단계로 깊어집니다." icon="academy" /><div className="academy-guide"><CharacterPortrait characterId="ari" expression="guide" position="right" /><p>각 단계는 단순 암기가 아닌 적용 훈련이야. 오답 시 힌트를 확인하고 재도전해 봐!</p></div><div className="room-list">{academyRooms.map((item) => { const terms = item.concepts.map((concept) => concept.term); return <button key={item.id} onClick={() => { setRoomId(item.id); setLevel(1); setChoice(null); setFlipped([]); }}><BookOpen size={24} /><div><strong>{item.title}</strong><small>{terms.join(" · ")}</small></div><span>{Math.max(0, ...terms.map((term) => save.mastery[term] ?? 0))}/4</span></button>; })}</div>{returnPoint && <button className="primary-button full-button" onClick={onReturn}>진행 중이던 Mission으로 복귀</button>}</div></AcademyTemplate>;
...
function RecordScreen({ save, onBack }: { save: SaveData; onBack: () => void }) {
  ...
  const avgTime = save.answerTimes.length ? Math.round(save.answerTimes.reduce((a, b) => a + b, 0) / save.answerTimes.length) : 0;
  ...
  return <AcademyTemplate><GameHUD missionTitle="나의 탐구 기록" level={save.level} exp={save.exp} onBack={onBack} /><div className="screen-scroll"><TemplateHeading eyebrow="LEARNING ANALYTICS" title="이해도와 성장 지표" description="단순 점수보다 어떤 개념을 보강할지 점검하세요." /><div className="analytics-cards"><Metric label="개념 이해도" value={understanding} /><Metric label="문항 정확도" value={accuracy} /><div className="time-card"><Clock size={26} /><strong>{avgTime}초</strong><span>평균 풀이 시간</span></div></div><div className="record-block"><h3>완료한 미션</h3><p>{save.completedMissions.length}/6</p><div className="feature-progress"><span style={{ width: `${save.completedMissions.length / 6 * 100}%` }} /></div></div><div className="record-block"><h3>복습 권장 개념</h3><div className="concept-tags">{save.reviewConcepts.length ? save.reviewConcepts.map((item) => <span key={item}>{item}</span>) : <span>현재 모든 개념을 통과했습니다.</span>}</div></div><div className="record-block"><h3>획득한 권리 카드</h3><p>{save.evidence.length}장 수집 · 근거 기반 판결 {save.decisionHistory.length}건 완료</p></div><div className="record-block"><h3>아카데미 탐구 글</h3><p>{Object.values(save.academyDrafts ?? {}).reduce((count, drafts) => count + Object.keys(drafts).length, 0)}편 저장됨 · 각 ROOM에서 언제든 다시 읽고 수정할 수 있습니다.</p></div><ExpressionLab /></div></AcademyTemplate>;
}
...
function ExpressionLab() {
  const sequences = {
    ari: ["guide", "think", "success", "surprise"],
    haeon: ["default", "warning", "skill", "success", "serious"],
...
function ActComplete({ onClose }: { onClose: () => void }) { return <div className="modal-backdrop completion-backdrop"><section className="act-complete" role="dialog" aria-modal="true" aria-labelledby="complete-title"><Sparkle size={40} weight="fill" /><span>ACT 1 COMPLETE</span><h2 id="complete-title">정식 인권수호관 임명</h2><CharacterPortrait characterId="haeon" expression="success" /><p>여섯 사건을 모두 해결하고 권리 카드 수집 훈련을 훌륭히 완수했습니다.</p><div><Medal size={22} />SKILL · 인권 렌즈 Lv.1 획득</div><blockquote>“수호관의 판단으로 우리 사회의 인권과 공정성 지표가 한 단계 성장했습니다.”</blockquote><small>다음 단원 · COMING SOON</small><button className="primary-button full-button" onClick={onClose}>미션 지도로 돌아가기</button></section></div>; }
